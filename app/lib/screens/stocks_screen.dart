...
class _StocksScreenState extends State<StocksScreen> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: _load,
        child: market == 'kr' ? _placeholder() : _body(),
      ),
    );
  }
  ...
  Widget _header(String title, String date, String updated, {bool stale = false, String? extra}) => Padding(
        ...
        child: Row(
          children: [
            ...
            const Spacer(),
            Text(
              '${date.isNotEmpty ? '기준 $date' : ''}${updated.isNotEmpty ? ' · 갱신 ${fmtUpdated(updated)}' : ''}${extra ?? ''}${stale ? ' (오프라인)' : ''}',
  ...
  List<Widget> _shortBody() {
    ...
    if (f != null) {
      ...
      out.add(Padding(
        ...
        child: Text('자금 ${fmtMoney(cap, market)} · 리스크 ${f.riskPct}% · 최대 ${settings.maxPositionsShort}종목 · 수량은 진입가=트리거가 가정',
            style: const TextStyle(fontSize: 11, color: Colors.grey)),
      ));
    ...
    final e = epF;
    out.add(_header('EP (에피소딕 피벗, 장중)', e?.date ?? '', e?.generatedAt ?? '',
        stale: e?.raw['_stale'] == true, extra: (e?.asofEt.isNotEmpty ?? false) ? ' · ${e!.asofEt} ET' : null));
  ...
  Widget _shortCard(ShortItem it, double cap, double riskPct) {
    ...
    return Card(
      child: InkWell(
        onTap: () => _openChart(it.ticker, it.name, it.exchange,
            trigger: it.triggerPrice, stop: it.stopPrice, horizon: 'short', qty: qty),
        child: Padding(
            ...
            children: [
              Row(
                children: [
                  ...
                  if (isEp)
                    Chip(
                      label: Text(it.triggered ? '진입가 도달' : '대기', style: const TextStyle(fontSize: 11)),
                      backgroundColor: it.triggered ? Colors.green.withValues(alpha: 0.2) : null,
  ...
  List<Widget> _midBody() {
    ...
    out.add(Padding(
      padding: const EdgeInsets.only(left: 4, bottom: 4),
      child: Text('자금 ${fmtMoney(cap, market)} · 리스크 ${f.riskPct}% · 최대 ${settings.maxPositionsMid}종목',
          style: const TextStyle(fontSize: 11, color: Colors.grey)),
    ));
    ...
    for (final it in f.items) {
      ...
      out.add(Card(
        child: InkWell(
          onTap: () => _openChart(it.ticker, it.name, it.exchange,
              trigger: it.entryPrice, stop: it.stopPrice, target: it.target2r, horizon: 'mid', qty: qty),
          child: Padding(
            padding: const EdgeInsets.all(10),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
      ...
      out.add(Card(
        child: Padding(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('배분표 · 장기 자금 ${fmtMoney(cap, market)} → 상위 ${f.holdN}종목 균등 (${fmtMoney(per, market)}씩)',
  ...
  void _openChart(String ticker, String name, String exchange,
      {double? trigger, double? stop, double? target, String horizon = 'short', int? qty}) {
